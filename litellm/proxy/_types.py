...
class CommonProxyErrors(str, enum.Enum):
    ...
    not_premium_user = (
        "This feature is not available in this OSS-only LiteLLM fork."
    )
    max_parallel_request_limit_reached = (
        "Crossed TPM / RPM / Max Parallel Request Limit"
    )
    missing_enterprise_package = (
        "This feature depends on LiteLLM Enterprise and is not available in this fork."
    )
    missing_enterprise_package_docker = (
        "This feature depends on proprietary enterprise code and is not available in this fork."
    )
